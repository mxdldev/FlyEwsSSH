...
<form name="myform" onSubmit='return CheckForm();' action="updateEadzone.aspx">
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
  <input type="hidden" name="eadzone.azid" value="${eadzone.azid}"/>
  ...
  <tr>
    ...
    <td><input name='eadzone.zatitle' type='text' id='aztitle' size='60' maxlength='60' value='${eadzone.zatitle}'> <font color='red'>*</font> </td>
  </tr>
  <tr>
    <td><strong>生成htm文件名：</strong></td>
    <td> <input name='eadzone.azjstitle' type='text' id='azjstitle' size='60' maxlength='100' value='${eadzone.azjstitle}'> <font color='red'>*</font>  </td>
  </tr>
  <tr>
    <td><strong>版位描述：</strong></td>
    <td><textarea name='eadzone.azintro' cols='50' rows='3' id='azintro'>${eadzone.azintro}</textarea></td>
  </tr>
  <tr>
    <td><strong>版位类型：</strong><br> 
   选择放置于此版位的广告类型</td>
    <td>
    <c:if test="${eadzone.aztype==0}">
     <input type='radio' name='eadzone.aztype' value='0'  id="ZoneType" checked> 矩形横幅&nbsp; 
    </c:if>
   <c:if test="${eadzone.aztype!=0}">
   <input type='radio' name='eadzone.aztype' value='0'  id="ZoneType" onclick='Change_Setting();'> 矩形横幅
   </c:if>
    <c:if test="${eadzone.aztype==1}">
    <input type='radio' name='eadzone.aztype' value='1' id="ZoneType" checked > 弹出窗口
    </c:if>
    <c:if test="${eadzone.aztype!=1}">
   <input type='radio' name='eadzone.aztype' value='1' id="ZoneType" onclick='Change_Setting();' > 弹出窗口
    </c:if>
     <c:if test="${eadzone.aztype==2}">
    <input type='radio' name='eadzone.aztype' value='2' id="ZoneType"  checked >漂浮移动
    </c:if>
    <c:if test="${eadzone.aztype!=2}">
      <input type='radio' name='eadzone.aztype' value='2' id="ZoneType"   onclick='Change_Setting();'>漂浮移动
    </c:if>
   <c:if test="${eadzone.aztype==3}">
     <input type='radio' name='eadzone.aztype' value='3' id="ZoneType"  checked>
        对联广告 
    </c:if>
    <c:if test="${eadzone.aztype!=3}">
       <input type='radio' name='eadzone.aztype' value='3' id="ZoneType"  onclick='Change_Setting();'>
        对联广告 
    </c:if>
      </td>
  </tr>
  <tr>
    ...
    <td>
  宽度: <input name='eadzone.azwidth' id="azwidth" size='5' maxlength='4'  value='${eadzone.azwidth}'>
   &nbsp;&nbsp;&nbsp;&nbsp; 高度:  <input name='eadzone.azheight' id="azheight" size='5' maxlength='4'  value='${eadzone.azheight}'>              <font color='red'>*</font>    
  ...
  <tr>
    ...
    <td>
    <c:if test="${eadzone.azshowtype==0}">
     <input name='eadzone.azshowtype' id="ShowType" type='radio' value='0' checked> 按权重随机显示&nbsp;&nbsp;权重越大显示机会越大。<br> 
    </c:if>
    <c:if test="${eadzone.azshowtype!=0}">
     <input name='eadzone.azshowtype' id="ShowType" type='radio' value='0'> 按权重随机显示&nbsp;&nbsp;权重越大显示机会越大。<br> 
    </c:if>
    <c:if test="${eadzone.azshowtype==1}">
      <input name='eadzone.azshowtype' id="ShowType" type='radio' value='1' checked> 按权重优先显示&nbsp;&nbsp;显示权重值最大的广告。<br>  
    </c:if>
    <c:if test="${eadzone.azshowtype!=1}">
      <input name='eadzone.azshowtype' id="ShowType" type='radio' value='1' > 按权重优先显示&nbsp;&nbsp;显示权重值最大的广告。<br>  
    </c:if>
    <c:if test="${eadzone.azshowtype==2}">
    <input name='eadzone.azshowtype' id="ShowType" type='radio' value='2' checked="checked"> 按顺序循环显示&nbsp;&nbsp;此方式仅对矩形横幅有效。
    </c:if>
    <c:if test="${eadzone.azshowtype!=2}">
     <input name='eadzone.azshowtype' id="ShowType" type='radio' value='2'> 按顺序循环显示&nbsp;&nbsp;此方式仅对矩形横幅有效。
    </c:if>
	</td>
  </tr>
  <tr>
    <td><strong>版位状态：</strong><br />
      设为活动的版位才能在前台显示</td>
    <td>
    <c:if test="${eadzone.azdisplay==0}">
     <input name='eadzone.azdisplay' type='checkbox'  value='0' checked="checked"> 活动版位    
    </c:if>
    <c:if test="${eadzone.azdisplay!=0}">
     <input name='eadzone.azdisplay' type='checkbox'  value='1' > 活动版位   
    </c:if>
   </td>
  </tr>
  <tr >
